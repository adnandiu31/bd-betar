    ...
    <div class="navbar-bottom">
        ...
        <div class="navbar-menu-wrapper d-flex flex-grow">
            <ul class="navbar-nav navbar-nav-right">
                @if(!isset($indent->approved_by_me_at))
                <li class="nav-item mr-2">
                    <form method="post" action="{{ route('centralEngineer.indents.status',$indent->id) }}">
                        @csrf
                        <button type="submit" class="btn {{ isset($indent->approved_by_ce_at) ? 'btn-danger' : 'btn-success' }}">
                            Mark as  {{ isset($indent->approved_by_ce_at) ? 'Pending' : 'Approved' }}
                        </button>
                    </form>
                </li>
                @endif
                <li class="nav-item">
                    <a href="{{ route('DSE.indents.index') }}" class="btn-shadow btn btn-danger">
