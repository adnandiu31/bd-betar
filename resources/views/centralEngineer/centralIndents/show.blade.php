        ...
        <div class="navbar-menu-wrapper d-flex flex-grow">
            <ul class="navbar-nav navbar-nav-right">
                <li class="nav-item mr-2">
                    <form method="post" action="{{ route('centralEngineer.indents.statusChange',$indent->id) }}">
                        @csrf
                ...
                @if ($indent->approved_by_me_at ==!null)
                    <li class="nav-item mr-2">
                        <form method="post" action="{{ route('centralEngineer.indents.finalApproved',$indent->id) }}">
                            @csrf
                ...
                @if ($indent->approved_by_me_at ==!null)
                    <li class="nav-item mr-2">
                        <form method="post" action="{{ route('centralEngineer.indents.approved',$indent->id) }}">
                            @csrf
                            <button type="submit" class="btn {{ $indent->approved_by_ce_at ? 'btn-success' : 'btn-danger' }}">
                                Mark as {{ $indent->approved_by_ce_at  ? 'Pending' : 'Approved' }}
                            </button>
                        </form>
                    </li>
                @endif
                <li class="nav-item">
                    <a href="{{ route('centralEngineer.indents.manufactureList') }}" class="btn-shadow btn btn-danger">
