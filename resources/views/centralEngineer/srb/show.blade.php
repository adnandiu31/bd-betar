    ...
    <div class="navbar-bottom">
        ...
        <div class="navbar-menu-wrapper d-flex flex-grow">
            <ul class="navbar-nav navbar-nav-right">
                @if(!isset($srb->approved_by_me_at))
                    <li class="nav-item mr-2">
                        <form method="post" action="{{ route('centralEngineer.srb.changeStatus',$srb->id) }}">
                            @csrf
                            <button type="submit" class="btn {{ isset($srb->approved_by_ce_at) ? 'btn-danger' : 'btn-success' }}">
                                Mark as  {{ isset($srb->approved_by_ce_at) ? 'Pending' : 'Approved' }}
                            </button>
                        </form>
                    </li>
                @endif
                <li class="nav-item">
                    <a href="{{ route('centralEngineer.srb.index') }}" class="btn-shadow btn btn-danger">
