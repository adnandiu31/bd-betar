        ...
        <div class="navbar-menu-wrapper d-flex flex-grow">
            <ul class="navbar-nav navbar-nav-right">
                    <li class="nav-item mr-2">
                        <form method="post" action="{{ route('directorGeneral.srb.changeStatus',$srb->id) }}">
                            @csrf
                            <button type="submit" class="btn {{ isset($srb->approved_by_dg_at) ? 'btn-danger' : 'btn-success' }}">
                                Mark as  {{ isset($srb->approved_by_dg_at) ? 'Pending' : 'Approved' }}
                            </button>
                        </form>
                    </li>
                <li class="nav-item">
                    <a href="{{ route('directorGeneral.srb.index') }}" class="btn-shadow btn btn-danger">
                                ...
                                @else
                                    <table id="datatable" class="align-middle mb-0 table table-borderless table-hover">
                                        ...
                                        @foreach($srb->parts as $key=>$srbPart)
                                            <tr>
                                                <td class="text-center text-muted">{{ $key + 1 }}</td>
                                                <td class="text-center">{{ $srbPart->part->name }}</td>
