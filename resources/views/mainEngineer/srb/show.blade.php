    ...
    <div class="navbar-bottom">
        ...
        <div class="navbar-menu-wrapper d-flex flex-grow">
            <ul class="navbar-nav navbar-nav-right">
                @if(!isset($srb->approved_by_dg_at))
                    <li class="nav-item mr-2">
                        <form method="post" action="{{ route('mainEngineer.srb.changeStatus',$srb->id) }}">
                            @csrf
                            <button type="submit" class="btn {{ isset($srb->approved_by_me_at) ? 'btn-danger' : 'btn-success' }}">
                                Mark as  {{ isset($srb->approved_by_me_at) ? 'Pending' : 'Approved' }}
                            </button>
                        </form>
                    </li>
                @endif
                <li class="nav-item">
                    <a href="{{ route('mainEngineer.srb.index') }}" class="btn-shadow btn btn-danger">
