    ...
    <div class="navbar-bottom">
        ...
        <div class="navbar-menu-wrapper d-flex flex-grow">
            <ul class="navbar-nav navbar-nav-right">
                @if(!isset($indent->approved_by_sh_at))
                    <li class="nav-item mr-2">
                        <form method="post" action="{{ route('stationHead.indents.status',$indent->id) }}">
                            @csrf
                            <button type="submit" class="btn {{ isset($indent->approved_by_si_at) ? 'btn-danger' : 'customButton' }}">
                                Mark as  {{ isset($indent->approved_by_sh_at) ? 'Pending' : 'Approved' }}
                            </button>
                        </form>
                    </li>
                @endif
                <li class="nav-item">
                    <a href="{{ route('stationHead.indents.index') }}" class="btn-shadow btn btn-danger">
