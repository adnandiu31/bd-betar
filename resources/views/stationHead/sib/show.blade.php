        ...
        <div class="navbar-menu-wrapper d-flex flex-grow">
            <ul class="navbar-nav navbar-nav-right">
                    <li class="nav-item mr-2">
                        <form method="post" action="{{ route('stationHead.sib.changeStatus',$sib->id) }}">
                            @csrf
                            <button type="submit" class="btn {{ isset($sib->approved_by_sh_at) ? 'btn-danger' : 'btn-success' }}">
                                Mark as  {{ isset($sib->approved_by_sh_at) ? 'Pending' : 'Approved' }}
                            </button>
                        </form>
                    </li>
                <li class="nav-item">
                    <a href="{{ route('stationHead.sib.index') }}" class="btn-shadow btn btn-danger">
                                    ...
                                    <table id="datatable" class="align-middle mb-0 table table-borderless table-hover">
                                        ...
                                        @foreach($sib->parts as $key=>$sibPart)
                                            <tr>
                                                ...
                                                {{-- <td class="text-center"><a href="{{route('stationHead.sib.part.instruments',$sibPart->id)}}" >{{ $sibPart->part->part->name }}</a></td> --}}
                                                <td class="text-center text-muted">{{ $key + 1 }}</td>
                                                {{-- <td class="text-center">{{ $sibPart->part->part->name }}</td> --}}
