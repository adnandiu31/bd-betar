    ...
    <div class="navbar-bottom">
        ...
        <div class="navbar-menu-wrapper d-flex flex-grow">
            <ul class="navbar-nav navbar-nav-right">
                @if(!isset($srb->approved_by_sh_at))
                    <li class="nav-item mr-2">
                        <form method="post" action="{{ route('stationIncharge.srb.changeStatus',$srb->id) }}">
                            @csrf
                            <button type="submit" class="btn {{ isset($srb->approved_by_si_at) ? 'btn-danger' : 'btn-success' }}">
                                Mark as  {{ isset($srb->approved_by_si_at) ? 'Pending' : 'Approved' }}
                            </button>
                        </form>
                    </li>
                @endif
                <li class="nav-item">
                    <a href="{{ route('stationIncharge.srb.index') }}" class="btn-shadow btn btn-danger">
                                    ...
                                    <table id="datatable" class="align-middle mb-0 table table-borderless table-hover">
                                        ...
                                        @foreach($srb->instruments as $key=>$srbInstrument)
                                            <tr>
                                                ...
                                                <td class="text-center">{{ $srbInstrument->quantity }}</td>
